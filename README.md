...
프론트엔드는 [KRDS 구현 강제 정책](docs/krds-implementation-policy.md)을 반드시 따른다.

- KRDS v1.1.0에 공식 컴포넌트가 있으면 자체 UI로 재구현하지 않음
- 사용자 정의 CSS는 배치·간격 등 최소 범위로 제한
- 토스·카카오페이식 UX는 정보구조와 흐름에만 적용하고 컴포넌트 외형은 KRDS 유지
- 과학·공학 시각화만 예외적으로 전문 시각화 라이브러리 사용
- 공식 용어 사용
- 번역투 금지
- 정책 위반 화면은 완료 또는 병합 대상으로 보지 않음
